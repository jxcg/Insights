...
enum SleepNightAggregator {
    ...
    // a sample close to the current sleep joins it, a long gap starts a new
    // one. Overlapping watch and phone samples land in the same session.
    ...
    // phone and watch can log the same minutes, so overlaps merge before
    // adding and every minute counts once. Deep and REM stay nil with no stage
    // data at all.
